        ...
        async function getSeat() {
            try {
                const seats = await fetch(
                    `${process.env.NEXT_PUBLIC_SERVER_PORT}/v1/showtimes/search/seats/${params.id}`,
                    {
                        credentials: "include",
                    }
                );
    ...
    return (
        <div className="h-full py-4">
                            ...
                            >
                                <Square
                                    ...
                                    selected={Boolean(
                                        selectedSeats?.find(
                                            (selectedSeats) =>
                                                seat.column === selectedSeats.column && seat.row === selectedSeats.row
                                        )
                                    )}
                                />
                            </button>
                    ...
                    <div>
                        ...
                        <div className="flex gap-3">
                            {selectedSeats.map(({ row, column }) => (
                                <SeatNumber key={`${row}-${column}`} row={row} column={column} />
                            ))}
                        </div>
                    </div>
                ) : null}
            ...
            <div className="flex gap-3 mt-7">
                <button
                    className="btn btn-sm bg-third font-lato font-semibold text-primary hover:text-third hover:bg-primary"
                    type="button"
                    onClick={() => resetSeats()}
                >
                    Reset
                ...
                {selectedSeats.length ? (
                    ...
                    >
                        <button className="btn btn-sm bg-secondary font-lato font-semibold text-third hover:text-primary hover:bg-third">
                            Confirm
                        </button>
                    </Link>
                ) : null}
