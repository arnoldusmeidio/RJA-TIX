...
import { FormTypeCreateReview, useFormCreateReview } from "./schema/Review.schema";
...
export default function ReviewModal({ movieData, idx }: Props) {
    ...
    const onSubmit: SubmitHandler<FormTypeCreateReview> = async (formData) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_PORT}/v1/movies/reviews`, {
                method: "POST",
                headers: {
    ...
    return (
        <FormProvider {...methods}>
            <form id={`review-${idx + 1}`} onSubmit={handleSubmit(onSubmit)}>
                ...
                <input type="checkbox" id={`review-tab-${idx + 1}`} className="modal-toggle" />
                <div className="modal bg-secondary" role="dialog">
                    <div className="modal-box h-3/3">
                        ...
                        <div className="py-4">
                            ...
                            <input
                                disabled={movieData?.reviews ? (movieData?.reviews.length !== 0 ? true : false) : false}
                                ...
                                max={5}
                                step={1}
                                defaultValue={movieData?.reviews.length !== 0 ? movieData.reviews[0].star : 3}
