...
export default function LoginForm() {
    ...
    const onSubmit: SubmitHandler<FormTypeLogin> = async (formData) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_PORT}/v1/auth/login`, {
                method: "POST",
                headers: {
    ...
    return (
        <>
            <form id="loginForm" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
                <label htmlFor="email" className="label-text font-semibold text-fourth align-middle text-base ms-2">
                    Email Address
                </label>
                <input
                    {...register("email")}
                ...
                <label htmlFor="password" className="label-text font-semibold text-fourth align-middle text-base ms-2">
                    Password
                </label>
                <input
                    {...register("password")}
                ...
                <div className="inline-flex align-middle mt-1">
                    ...
                    <span className="label-text font-medium text-fourth ms-2">Remember me</span>
                </div>
            </form>
            <button
