...
import { FormTypeCreatePayment, useFormCreatePayment } from "@/components/schema/PaymentSchema";
...
export default function Page() {
    ...
    if (Number(point) && Number(voucherValue) && Number(adminVoucherDiscount)) {
        ...
        const discountedPrice1 = priceMinusPoint - ((initialPrice - Number(point)) * Number(voucherValue)) / 100;
        totalPrice = discountedPrice1 - (discountedPrice1 * Number(adminVoucherDiscount)) / 100;
    } else if (Number(point) && Number(voucherValue)) {
        totalPrice = initialPrice - Number(point) - ((initialPrice - Number(point)) * Number(voucherValue)) / 100;
    } else if (Number(point) && Number(adminVoucherDiscount)) {
        totalPrice =
            initialPrice - Number(point) - ((initialPrice - Number(point)) * Number(adminVoucherDiscount)) / 100;
    } else if (Number(voucherValue) && Number(adminVoucherDiscount)) {
        totalPrice =
            initialPrice -
            (initialPrice * Number(voucherValue)) / 100 -
            ((initialPrice - (initialPrice * Number(voucherValue)) / 100) * Number(adminVoucherDiscount)) / 100;
        ...
        totalPrice = initialPrice - (initialPrice * Number(adminVoucherDiscount)) / 100;
    }
                ...
                <div className="card-body">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <div className="point-voucher">
                            <form id="payment" className="flex flex-col gap-2" onSubmit={handleSubmit(onSubmit)}>
                                <label className="font-inter font-medium text-fourth" htmlFor="points">
                                    Points
                                </label>
                                <select
                                ...
                                <label className="font-inter font-medium text-fourth" htmlFor="voucher">
                                    Voucher
                                </label>
                                <select
                                    className="select w-full bg-primary border-third focus:border-fourth border-2"
                                ...
                                <div className="flex flex-col">
                                    <label className="font-inter font-medium text-fourth mb-2" htmlFor="promoCode">
                                        PROMO CODE
                                    </label>
                                    <input
                                        type="text"
                                    ...
                                    {adminVouchers ? (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() =>
                                                setValue("adminVoucherDiscount", Number(adminVouchers.discount))
                                            }
                                        >
                        ...
                        <div className="seat-payment bg-primary h-fit rounded-lg p-3">
                            ...
                            <div className="font-inter font-medium text-base">
                                Rp.{bookingData[0].price},00 x <span className="text-third">{bookingData.length}</span>
                            </div>
                            ...
                            <div className="font-inter font-semibold mt-3 text-third">Your Balance:</div>
                            <div className="font-inter font-medium text-base">
                            ...
                            <div className="mt-3">
                                ...
                                <div
                                    className={`${
                                        Number(point) || Number(voucherValue) || Number(adminVoucherDiscount)
                                            ? "line-through"
                                            : null
                                    }`}
                                >
                            ...
                            {Number(point) ? <div>{`You save ${initialPrice - totalPrice},00`}</div> : null}
                            <button
