...
import { FormProvider, SubmitHandler, useFieldArray, useFormContext } from "react-hook-form";
import { FormTypeCreateShowtime, useFormCreateShowtime } from "./schema/ShowtimeSchema";
...
export default function CreateShowtime() {
    ...
    const onSubmit: SubmitHandler<FormTypeCreateShowtime> = async (formData) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_PORT}/v1/showtimes`, {
                method: "POST",
                headers: {
...
const SelectMovie = () => {
    ...
    return (
        <div className="flex flex-col mb-4">
            <label className="font-inter font-medium text-xl text-third pb-1" htmlFor="movie">
                Movie
            </label>
            <select
...
const SelectStudio = () => {
    ...
    useEffect(() => {
        async function getCinema() {
            try {
                const movie = await fetch(`${process.env.NEXT_PUBLIC_SERVER_PORT}/v1/cinemas/managers`, {
                    credentials: "include",
                });
                const data = await movie.json();
    ...
    return (
        <div className="flex flex-col mb-4">
            <label className="font-inter font-medium text-xl text-third pb-1" htmlFor="studio">
                Studio number
            </label>
            <select
                className="select bg-primary border-fourth focus:border-third border-1 rounded-lg"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2">
                {fields.map((showtime, showtimeIndex) => (
                    <div className="mx-auto" key={showtime.id}>
                        <label className="font-inter font-medium text-xl text-third pb-1" htmlFor="date">
                            Date and time
                        </label>
